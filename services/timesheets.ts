...
export class TimesheetsService {
    ...
    static async getTimesheetEntries(
        ...
        try {
            const buildUrl = (base: string): URL => {
            const url = new URL(base);

            if (employeeId != null) url.searchParams.set('employee', String(employeeId));
            if (all_users)          url.searchParams.set('all_users', String(all_users));

            // startDate/endDate hanno priorità su month/year
            ...
            const workedUrl       = buildUrl(`${backendUrl}/api/timesheets/timeworks`);
            const noWorkedUrl     = buildUrl(`${backendUrl}/api/timesheets/`);
            noWorkedUrl.searchParams.set('no_worked_hours', 'true');

            const [workedRes, noWorkedRes] = await Promise.all([
            fetch(workedUrl.toString(),   { headers: { Authorization: `Bearer ${token}` } }),
            fetch(noWorkedUrl.toString(), { headers: { Authorization: `Bearer ${token}` } }),
            ]);

            const [data, noWorkedHoursData] = await Promise.all([
            workedRes.json(),
            noWorkedRes.json(),
            ]);
